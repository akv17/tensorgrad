...
*Also you may drop-in replace `tensorgrad` with `torch` (and vice-versa) by replacing first line with `import torch as tensorgrad`.*
...
In all the tests `torch` always runs on `cpu` device even if `tensorgrad` runs on `cuda` via `DEVICE=cuda`.  
